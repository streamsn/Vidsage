...
export function AccountBar({ session, credits }: { session: Session; credits: CreditsResponse | null }) {
  return (
    <div className="flex flex-wrap items-center justify-center gap-x-1 gap-y-1 text-sm text-slate-500">
      <span className="break-all">Signed in as {session.user.email}</span>
      {credits && <span>· {credits.creditBalance} credits remaining</span>}
      <span>
        ·{" "}
        <button onClick={() => supabaseBrowser.auth.signOut()} className="underline hover:text-slate-700">
          Sign out
        </button>
      </span>
    </div>
